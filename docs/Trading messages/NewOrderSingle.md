...
1     |  `Account`|The FIX platform account who placed the order. If not specified, then the platform default account is used.<br>In the system, this value must be specified in **Accounts allowed for session** and belong to a group from **Groups allowed for session**; otherwise, the order is rejected| N |string
55| `Symbol` |Symbol name| Y| string
...
60| `TransactTime`&nbsp;| Time at which this order request was initiated. <br>Time and date are represented in UTC in YYYMMDD-HH:SS.sss format|Y|time
 ...
 40 | `OrdType`|Order type. The following types are supported: <br> <ul><li>1 = Market</li><li>2 = Limit</li></ul>|Y|char
 44| `Price` |Order price.<br>Required only for Limit orders (`OrdType` = 2)|N|price
 59| `TimeInForce`|How long the order remains in effect.<br><ul><li>1 = GTC</li><li>2 = IOC</li><li>3 = FOK</li></ul>For market orders (`OrdType` = 1), only 2 and 3 are valid.<br>For limit orders (`OrdType` = 2), all values are valid| N| char
